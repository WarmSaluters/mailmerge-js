...
const MAILMERGE_AUTH_SERVER_URL = process.env.AUTH_SERVER_URL ?? "https://auth.mailmerge-js.dev";
...
export async function authorize(strategy?: "local" | "server"): Promise<Auth.OAuth2Client> {
    ...
    if (strategy === "server") {
        ...
        authorizer = Config.googleCredentialsJSON ? LocalAuthorizer : MailmergeServerAuthorizer;
    }
...
export const MailmergeServerAuthorizer: Authorizer = {
    ...
    getAuthorizedClient: async () => {
        ...
        if (isTokenExpired(token)) {
            ...
            const tokenResponse = await axios.get(`${MAILMERGE_AUTH_SERVER_URL}/refresh?token=${Config.gmailToken}`);
...
const isTokenExpired = (token: { expiry_date: number }, toleranceMS: number = 10000) => {
