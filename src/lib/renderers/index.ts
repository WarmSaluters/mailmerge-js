...
import { NunjucksRenderer } from "./nunjucks.js";
...
export const supportedRenderers: Record<RendererTag, RendererInfo> = {
  ...
  "engine/nunjucks": {
    name: "nunjucks",
    description: "Nunjucks is a non-ai template engine inspired from Jina2",
    aliases: ["nunjucks"],
  },
};

export const getRenderer = (tagOrAlias: string): Renderer => {
  const resolved = Object.keys(supportedRenderers).find(
    (tag) =>
      tag === tagOrAlias ||
      supportedRenderers[tag as RendererTag].aliases?.includes(tagOrAlias)
  );
  if (!resolved) {
  ...
  switch (resolved as RendererTag) {
    ...
    case "engine/nunjucks":
      return new NunjucksRenderer("llama3");
    default:
      throw new Error(`Renderer not found: ${tagOrAlias}`);
  }
};
